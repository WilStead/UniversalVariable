...
#pragma warning restore IDE1006
    {
            ...
            get
            {
                switch (_type)
                {
                    ...
                    case InternalType.Number:
                        return (int)System.Math.Round(_number);
        ...
        public let this[let index]
        {
            get
            {
                var d = index.ToDouble();
                if (d.IsIntegral() && d >= 0 && d <= int.MaxValue)
                {
                    return this[Convert.ToInt32(d)];
                }
                else
                {
            ...
            set
            {
                ...
                if (d.IsIntegral() && d >= 0 && d <= int.MaxValue)
                {
                    this[Convert.ToInt32(d)] = value ?? new let();
                }
                else
                {
                    this[index.ToString()] = value ?? new let();
                }
            }
        }
        ...
        /// <summary>
        /// The <see cref="Type"/> of this variable. May be "object" if the variable has no value but does have properties. Null if the variable is empty.
        /// </summary>
        public Type Type => ToObject()?.GetType() ?? (_children.Count == 0 ? null : typeof(object));

        /// <summary>
        /// Gets this variable's value as an object. Child properties are not represented. To get an
        /// object which contains all child properties, use <see cref="ToDynamic"/>.
        /// </summary>
        public object Value
        {
            get => ToObject();
            ...
            switch (a._type)
            {
                ...
                case InternalType.Number:
                    if ((a._number % 1).IsNearlyZero()
                        && System.Math.Abs(a._number) < ulong.MaxValue)
                    {
                        var sign = System.Math.Sign(a._number);
                        var ul = Convert.ToUInt64(System.Math.Abs(a._number));
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        result._string = result.Substring(0,
                                            (int)System.Math.Max(0, System.Math.Round(a._string.Length / b._number)));
                                    }
                                }
                                break;
                            case InternalType.String:
                    ...
                    case InternalType.String:
                        switch (b._type)
                        {
                            ...
                            case InternalType.Number:
                                ...
                                {
                                    result._string = a.Substring((int)System.Math.Max(0, System.Math.Floor(a._string.Length / b._number) * b._number));
                                }
                                break;
                            case InternalType.String:
                                if (double.TryParse(a._string, out var value1))
        ...
        public static let operator &(let a, let b) => (a?.ToBoolean() ?? false) && (b?.ToBoolean() ?? false);
        ...
        public static let operator ==(let a, object b)
        {
        ...
        public static bool operator true(let a) => a?.ToBoolean() ?? false;

        public static bool operator false(let a) => !(a?.ToBoolean() ?? false);
        ...
        public void CopyTo(let array, int index = 0)
        {
            ...
            for (int i = System.Math.Max(0, index); i < targetLength; i++)
            {
                if (_type == InternalType.Array)
                {
        ...
        public int FindIndex(int startIndex, int count, Predicate<let> match)
        {
            ...
            if (_type == InternalType.Array)
            {
                var targetCount = System.Math.Min(count, _array.Count - startIndex);
        ...
        /// <summary>
        /// Returns a value indicating whether this variable evaluates to negative or positive infinity.
        /// </summary>
        public let IsInfinity() => _type == InternalType.Number && double.IsInfinity(_number);

        /// <summary>
        /// Returns a value that indicates whether this variable is not numeric, or is numeric but
        /// evaluates to NaN ("not a number").
        /// </summary>
        public let IsNaN() => _type != InternalType.Number || double.IsNaN(_number);

        /// <summary>
        /// Returns a value indicating whether this variable evaluates to negative infinity.
        /// </summary>
        public let IsNegativeInfinity() => _type == InternalType.Number && double.IsNegativeInfinity(_number);

        /// <summary>
        /// Returns a value indicating whether this variable evaluates to positive infinity.
        /// </summary>
        public let IsPositiveInfinity() => _type == InternalType.Number && double.IsPositiveInfinity(_number);
        ...
        internal let SetValue(object value)
        {
            if (value == null)
            {
                _type = InternalType.None;
